...
def add_round_key(state: [[int]], key_schedule: [[[int]]], roun: int):
    round_key = key_schedule[roun]
...
def bytes_to_state(data: bytes) -> [[int]]:
...
def aes_Encrypt(data: bytes, key: bytes) -> bytes:
    ...
    for r in range(1, nr):
        ...
        shift_rows(state)
        mix_columns(state)
        add_round_key(state, key_schedule, r)
...
def string_to_hex(input_string):
    return input_string.encode('utf-8').hex().ljust(32, '0')


def hex_to_string(hex_string):
    return bytes.fromhex(hex_string).decode('utf-8').split('\x00', 1)[0]
...
if __name__ == "__main__":
    ...
    text = 'Olá mundo!'
    key = 'bankai'
    hextext = string_to_hex(text)
    hexkey = string_to_hex(key)
    print(hextext)
    plaintext = bytearray.fromhex(hextext)
    # plaintext = bytearray.fromhex('00112233445566778899aabbccddeeff')
    # bytekey = bytearray.fromhex('000102030405060708090a0b0c0d0e0f')
    bytekey = bytearray.fromhex(hexkey)
    # expected_ciphertext = bytearray.fromhex('69c4e0d86a7b0430d8cdb78070b4c55a')
    expected_ciphertext = bytearray.fromhex('7dbdc79f5f9419683c6b4d8883446b3a')
    ciphertext = aes_Encrypt(plaintext, bytekey)
    print(bytes_to_hex(ciphertext))
    recovered_plaintext = aes_Decrypt(ciphertext, bytekey)
    print(bytes_to_hex(recovered_plaintext))
    print(hex_to_string(bytes_to_hex(recovered_plaintext)))
    ...
    recovered_plaintext = aes_Decrypt(ciphertext, key)
...
# --------------Testar Força de Criptografia---------------
def BruteForceAttack(encrypted_text):

    if encrypted_text is None:
        print("Erro: Texto criptografado inválido")
    ...
    for _ in range(256):
        ...
        try:
            decrypted_block = aes_Decrypt(encrypted_text, key_bytes)
            print(hex_to_string(bytes_to_hex(decrypted_block)))
            if decrypted_block:
                decrypted_text = hex_to_string(bytes_to_hex(decrypted_block))
                print(decrypted_text)
                if decrypted_text == "YourPlainText":
                    print(f"Chave encontrada: {key}")
                    return key
        finally:
            return "Criptografia segura!"
